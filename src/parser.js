...
export function parseHLSL(code) {
  ...
  let match;
  while ((match = typePattern.exec(cleaned)) !== null) declaredVars.set(match[2], match[1]);
  ...
  for (const line of cleaned.split(/\n/)) {
    for (const m of [...line.matchAll(/\b([A-Za-z_]\w*)\s*([+\-*\/!<>]?)=/g)]) {
      if (m[2] === "" && line[m.index + m[0].length] !== "=") {
        const name = m[1];
        if (!declaredVars.has(name) && !HLSL_KEYWORDS.has(name) && !HLSL_TYPES.has(name)) assignedUndeclared.add(name);
      }
    }
  }
  ...
  while ((match = identPattern.exec(cleaned)) !== null) allIdents.add(match[1]);
  ...
  for (const line of cleaned.split("\n")) {
    ...
    if (eqIdx > 0 && trimmed[eqIdx-1] !== "!" && trimmed[eqIdx-1] !== "<" && trimmed[eqIdx-1] !== ">" && trimmed[eqIdx+1] !== "=") {
      for (const m of trimmed.substring(eqIdx+1).matchAll(/(?<!\.)(\b[A-Za-z_]\w*)\b/g)) readVars.add(m[1]);
    } else {
      for (const m of trimmed.matchAll(/(?<!\.)(\b[A-Za-z_]\w*)\b/g)) readVars.add(m[1]);
    }
  }
  ...
  while ((match = swizzlePattern.exec(cleaned)) !== null) {
    const name = match[1], swizzle = match[2];
    let dim = 1;
  ...
  const skipSet = new Set([...HLSL_KEYWORDS, ...HLSL_TYPES, ...HLSL_BUILTINS, ...UE_BUILTINS, ...samplerVars, ...declaredVars.keys()]);
  ...
  for (const name of allIdents) {
    ...
    if (isRead && !declaredVars.has(name)) {
      if (isAssigned) {
        ...
        if (firstAssignMatch && firstAssignMatch.index <= firstReadIdx + name.length + 3) { outputs.push({ name }); continue; }
      }
      let inferredType = "float", nodeType = "ScalarParameter", defaultValue = "0.0";
      ...
      if (nameLower === "time" || nameLower === "gametime") { nodeType = "Time"; inferredType = "float"; defaultValue = ""; }
      else if (nameLower.includes("uv") || nameLower.includes("texcoord") || nameLower.includes("coord")) { nodeType = "TextureCoordinate"; inferredType = "float2"; defaultValue = ""; }
      else if ((nameLower.includes("color") || nameLower.includes("colour")) && nodeType !== "VectorParameter") { nodeType = "VectorParameter"; inferredType = "float3"; defaultValue = "(R=0.0,G=0.0,B=0.0,A=1.0)"; }
  ...
  if (retMatch) { const t = declaredVars.get(retMatch[1]); if (t === "float" || t === "float1") returnType = "CMOT_Float1"; else if (t === "float2") returnType = "CMOT_Float2"; else if (t === "float3") returnType = "CMOT_Float3"; else if (t === "float4") returnType = "CMOT_Float4"; }
